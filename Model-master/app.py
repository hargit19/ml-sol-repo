...
def send_mail(mailId, msgBody):
    msg = Message("Hey", sender="noreply@gmail.com", recipients=[mailId])
    msg.body = msgBody
    mail.send(msg)
    return "Mail sent"
...
def process_file():
    ...
    try:
        ...
        x_input = df_filled.shape[1]
        # print(df_filled.shape[1])
        X_last_day = val_norm[-n_input:]
        X_last_day = X_last_day.reshape(1, n_input, x_input)
...
def process_file2():
    ...
    try:
        ...
        # Round all values to 2 decimal places
        df_filled = df_filled.round(2)
        # df_filled.reset_index(level="date", inplace=True)
        # Drop the 'time' column as it's no longer needed
        app.logger.debug(df_filled)
